            ...
            <table class='sortable display responsive' id='Tableau' style="width:100%">
                ...
                <tbody>
                    ...
                    <?php $a = 0; foreach ($query1 as $value1) { ?>
                        ...
                        <tr>
                            <td><span style="font-weight: bold;font-size: 20px;">★ </span><a href="<?= $link; ?>" target='_blank' style="font-weight: bold"><?= get_field("report_title", $value1->ID); ?></a></td>
                            <td style="font-weight: bold"><?= get_field("year", $value1->ID); ?></td>
                            <td style="font-weight: bold"><?= get_field("author_name", $value1->ID);   ?></td>
                            <td style="font-weight: bold"><?= ucfirst( get_field("country", $value1->ID) ); ?></td>

                            <?php  $ii=0;?>
                            <?php if($tags = explode("; ",get_field("tags", $value1->ID))):?>
                                
                                <td style="font-weight: bold"><ul><?php foreach ($tags as $tag) { ?> <li> <?php echo ucfirst($tag);  array_push($allTags,strtolower($tag)); ?> </li> <?php } ?></ul></td>
                            <?php else: ?>
                                <td style="font-weight: bold"><?= get_field("tags", $value1->ID);?></td>
                            <?php endif; ?>
                        </tr>
